...
class Claim(TimeStampedModel, AuthStampedModel):
    ...
    def __unicode__(self):
        return "Claim: #%s, date:%s, customer:%s" % (self.pk, formats.date_format(self.claim_date, 'DATE_FORMAT_SHORT'), str(self.customer))
...
class ClaimStatus(TimeStampedModel, AuthStampedModel):
    ...
    def __unicode__(self):
        return "Claim Status: %s, date=%s, claim #%s" % (self.get_status_display(), formats.date_format(self.date, 'j F, Y'), self.claim.pk)
